...
export function useAddressMutations() {
  ...
  return {
    create: useMutation({ mutationFn: (b: AddressInput) => addressesApi.create(b),
      onSuccess: () => done("Address added"), onError: (e: Error) => toast.error(e.message) }),
    update: useMutation({ mutationFn: (p: { id: string; body: AddressInput }) => addressesApi.update(p.id, p.body),
