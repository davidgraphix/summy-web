...
import type { InitializePaymentRequest, Payment, PaymentInitialization } from "@/types/models";

export const paymentsApi = {
  initialize: (body: InitializePaymentRequest) =>
    api.post<PaymentInitialization>("/payments/initialize", body),
  verify: (reference: string) => api.post<Payment>(`/payments/verify/${encodeURIComponent(reference)}`),
  retry: (paymentId: string) => api.post<PaymentInitialization>(`/payments/${paymentId}/retry`),
