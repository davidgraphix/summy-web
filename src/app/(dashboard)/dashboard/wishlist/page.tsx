            ...
            <Card key={item.id}>
              <CardContent className="flex gap-4 p-4">
                <div className="grid h-20 w-20 shrink-0 place-items-center overflow-hidden rounded-xl bg-muted text-muted-foreground/40">
                  {item.imageUrl ? <img src={item.imageUrl} alt="" className="h-full w-full object-cover" /> : <Package size={28} />}
                </div>

                <div className="flex min-w-0 flex-1 flex-col">
                  <Link href={`/products/${item.productSlug}`} className="line-clamp-2 font-medium hover:text-primary">
                    {item.productName}
                  </Link>
                  <p className="mt-0.5 font-extrabold">{item.currentPriceFormatted}</p>
                  {item.hasPriceDropped && (
                    <p className="text-xs font-medium text-success">Price dropped since you saved this</p>
                  )}

                  <div className="mt-auto flex flex-wrap items-center gap-2 pt-3">
                    {/* Uses POST /cart/from-wishlist/{productId} */}
                    <Button size="sm" onClick={() => cart.addFromWishlist.mutate(item.productId)}
                      disabled={cart.addFromWishlist.isPending || !item.isPurchasable}>
                      <ShoppingCart size={15} />
                      {item.isPurchasable ? "Move to cart" : "Out of stock"}
                    </Button>
