...
>(({ className, ...props }, ref) => (
  <TabsPrimitive.List ref={ref}
    className={cn("flex gap-1 overflow-x-auto border-b border-border", className)} {...props} />
));
...
>(({ className, ...props }, ref) => (
  <TabsPrimitive.Trigger ref={ref}
    className={cn(
      "relative whitespace-nowrap px-3.5 py-2.5 text-sm font-medium text-muted-foreground transition-colors",
...
>(({ className, ...props }, ref) => (
  <TabsPrimitive.Content ref={ref}
    className={cn("pt-5 focus-visible:outline-none", className)} {...props} />
));
