...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger ref={ref}
    className={cn(
      "flex h-10 w-full items-center justify-between gap-2 rounded-xl border border-input bg-card px-3 text-sm outline-none transition-colors",
      "focus:border-primary focus:ring-2 focus:ring-ring data-[placeholder]:text-muted-foreground disabled:opacity-50",
      className
    )} {...props}>
    {children}
    <SelectPrimitive.Icon asChild><ChevronDown size={15} className="shrink-0 opacity-60" /></SelectPrimitive.Icon>
  ...
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content ref={ref} position={position}
      className={cn(
        ...
        "data-[state=open]:animate-[content-in_120ms_ease-out]",
